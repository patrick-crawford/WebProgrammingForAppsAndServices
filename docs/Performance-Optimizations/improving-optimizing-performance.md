...
```jsx
<Image
  src="https://www.senecacollege.ca/content/dam/projects/seneca/campus-photos/magna-hall_tile.jpg"
  className={styles.headerImage}
  width={600}
  height={386}
/>
...
```console
Error: Invalid src prop (https://www.senecacollege.ca/content/dam/projects/seneca/campus-photos/magna-hall_tile.jpg) on `next/image`, hostname "www.senecacollege.ca" is not configured under images in your `next.config.js`
See more info: https://nextjs.org/docs/messages/next-image-unconfigured-host
```

If we navigate to the link in ["more info"](https://nextjs.org/docs/messages/next-image-unconfigured-host), we will see that the error occurred because the "src" value uses a hostname that isn't defined in "images.domains" in "next.config.js". This is done to ensure that only images from approved domains are able to use the Next.js image optimization API.

To solve this problem, open the "next.config.js" file, and update the **nextConfig** object to include an "images" property with a list of approved domains:
...
Next.js supports ["Lazy Loading"](https://developer.mozilla.org/en-US/docs/Web/Performance/Lazy_loading) for external libraries with "import" as well as [images](https://nextjs.org/docs/api-reference/next/image#loading) and [pages](Pages-Routing/layouts-and-pages.md). In larger apps, this can have an impact on metrics such as Largest Contentful Paint (LCP) and First Input Delay (FID) due to the smaller bundle size that is required on the first page load.
...
5. Add an ["onSelect"](https://react-bootstrap.github.io/components/accordion/#accordion-props) event to the &lt;Accordion className="mt-4"&gt;...&lt;/Accordion&gt; element so that we can execute code once the user opens the accordion.
